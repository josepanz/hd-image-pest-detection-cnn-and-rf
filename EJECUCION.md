...
- **Contaminación por `nodata` — corregido**: las bandas de reflectancia WUR marcan fuera-de-cobertura con `nodata=-10000.0`. Al recortar por polígono de parcela, una porción importante de los píxeles cae en esa zona (ejemplo medido: parcela `PlotID=1`, fecha `2023-05-18`, banda `red` → **44.6% de los píxeles del recorte son `-10000`**, no un caso aislado de borde). La normalización dividía por el máximo de la imagen (que ignora los negativos), así que esos píxeles `-10000` terminaban en valores post-normalización del orden de `-2.6 millones` en vez de recortarse/enmascararse. Estaba tanto en `extract_data_to_img.py` (entrenamiento) como en `inference_utils.py` (inferencia) - **corregido en los 3 lugares** (extracción de entrenamiento, recorte por parcela e imagen completa en inferencia): los píxeles `nodata` se ponen en `0` antes de apilar/normalizar. Validado: la parcela de arriba pasó de `min=-2.6M` a `min=0.0, max=1.0`. **Como cambia la entrada real que ve el modelo, los checkpoints existentes en `best_models/` quedaron desactualizados y hace falta reentrenarlos** (ver "Secuencia completa" al principio de este documento) - no aplica a RGB (sin `nodata` declarado en esos TIFF).

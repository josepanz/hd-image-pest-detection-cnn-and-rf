...
best_models/ y corren inference_models.py::run_unified_inference de punta a punta
sobre muestras sintéticas (ver conftest.py). Son lentos (cargan TensorFlow y modelos
...
from src.pest_detection_models import inference_models
...
def _run_and_load_json(monkeypatch, tmp_path, path, model_path, arch_type, loss="fl"):
    monkeypatch.setattr(inference_models, "BASE_DIR", str(tmp_path))
    inference_models.run_unified_inference(str(path), model_path, threshold=0.5, arch_type=arch_type, loss=loss)
...
def test_cnn_multiespectral_inference_end_to_end(monkeypatch, tmp_path, ms_sample_dir):
    model_path = _model_path("best_model_final_MULTIESPECTRAL_focal_loss.keras")

    results = _run_and_load_json(monkeypatch, tmp_path, ms_sample_dir, model_path, arch_type="cnn")
...
def test_cnn_rgb_inference_end_to_end_with_bare_file_path(monkeypatch, tmp_path, rgb_sample_file):
    ...
    results = _run_and_load_json(monkeypatch, tmp_path, rgb_sample_file, model_path, arch_type="cnn")

    assert len(results) == 1
    assert results[0]["prediccion"] in ("Plaga", "Sana")


def test_random_forest_multiespectral_inference_end_to_end(monkeypatch, tmp_path, ms_sample_dir):
    ...
    results = _run_and_load_json(monkeypatch, tmp_path, ms_sample_dir, model_path, arch_type="rf", loss="fl")

    assert len(results) == 1
    assert results[0]["prediccion"] in ("Plaga", "Sana")


def test_random_forest_rgb_inference_end_to_end(monkeypatch, tmp_path, rgb_sample_file):
    ...
    results = _run_and_load_json(monkeypatch, tmp_path, rgb_sample_file, model_path, arch_type="rf", loss="fl")
